...
19. **claude-sdk backend (P4, 2026-08-21)** — fifth backend using the Claude Agent SDK (Python, in-process) alongside `claude-cli` (which stays default). Headline motivation: **custom tools** — expose Karakeep search, Obsidian vault, MusicGrabber as native tools the model calls directly instead of via shell. Also gains structured streaming, clean cancel (no process groups), per-run token/cost, session forking, `canUseTool` permission callbacks (unused for now — force + sudo modal is the decided default). ~1 day using `backends/cursor_sdk.py` as the template; `/backend claude-sdk`. Build when claude-cli hurts or when custom tools are wanted.
20. ~~**launchd install path (2026-08-21)**~~ ✅ — macOS LaunchAgent alongside the Linux systemd unit: `deploy/launchd/*.plist` templates + `scripts/install-launchd-service.sh` (user-level, `--uninstall`), `scripts/launchd-exec.sh` to source `.env` (launchd has no `EnvironmentFile=`), and a shared `scripts/deploy.sh` that detects the init system and guards against `WatchPaths` double-firing on flag deletion. No launchd equivalent for `MemoryMax`; LaunchAgent needs the user logged in.
21. ~~**Outbound attachments (2026-08-21)**~~ ✅ — agents send files back: `[[attach: /abs/path]]` in a reply becomes a Discord upload (`src/zen_agent_bot/outbound.py` + `send_files` job callback). Markers are always stripped; files must resolve inside the workspace / `data/` / temp dirs (symlinks resolved, traversal fails closed) and secret-looking names are refused. Caps: 10 files, 8 MiB (`OUTBOUND_MAX_FILES` / `OUTBOUND_MAX_BYTES`). Prompt hint injected so agents know the syntax. Telegram falls back to listing paths — upload support there is the follow-up.
