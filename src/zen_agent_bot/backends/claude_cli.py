...
class ClaudeCliConfig:
    command: str = "claude"
    force: bool = True  # → --dangerously-skip-permissions
    model: str | None = None
...
def _format_progress(*, phase: str, text: str, max_len: int = 1800) -> str:
    ...
    header = f"{icons.get(phase, '⏳')} **Claude Code…**"
    if not text:
        return header
...
def _usable_resume(session_id: str | None) -> str | None:
    """Skip foreign backend session ids (e.g. OpenRouter or-*)."""
    if not session_id:
        return None
    if session_id.startswith("or-"):
        return None
    return session_id


class ClaudeCliBackend:
    """Claude Code CLI (`claude -p`) — Pro/Max subscription, coding agent with tools."""
    ...
    def _resolve_bin(self) -> str:
        ...
        if not path:
            raise FileNotFoundError(
                f"Claude Code binary not found: {self.config.command} "
                "(install from https://claude.ai/code and run `claude` once to log in)"
            )
        return path
        ...
        try:
            ...
            if cancel_event:
                ...
                if comm_task not in done:
                    ...
                    await terminate_process(proc)
                    return AgentRunResult(
                        text="Claude Code run timed out.",
                        session_id=session_id,
                        exit_code=124,
                        error="timeout",
                    )
        ...
        try:
            while True:
                ...
                if etype == "system" and event.get("subtype") == "init":
                    ...
                    if chunk:
                        # Full turn snapshot (partials come via stream_event)
                    ...
                    if delta:
                        ...
                        phase = "writing"
                        await on_progress(_format_progress(phase=phase, text=assistant_text))
                elif etype in ("tool_call", "tool", "function_call") or (
                    etype == "user" and _has_tool_result(event)
                ):
                    phase = "tool"
                    await on_progress(_format_progress(phase=phase, text=assistant_text))
                elif etype == "result":
        ...
        if stdout:
            try:
                ...
                if payload.get("is_error"):
                    ...
                    if not text:
                        text = str(payload.get("error") or "Claude Code returned an error.")
            except json.JSONDecodeError:
                text = stdout
...
def _stream_delta_text(event: dict) -> str:
    """Extract text from Claude stream_event / partial message payloads."""
    ...
    if isinstance(message, dict):
        return _assistant_text({"message": message})
    return ""


def _has_tool_result(event: dict) -> bool:
    message = event.get("message") or {}
    parts = message.get("content") or []
    for part in parts:
